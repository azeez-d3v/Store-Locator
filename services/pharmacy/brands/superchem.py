...
class SuperChemHandler(BasePharmacyHandler):
    """Handler for SuperChem Pharmacy stores"""
    
    def __init__(self, pharmacy_locations):
        super().__init__(pharmacy_locations)
        self.brand_name = "superchem"
        self.base_url = "https://www.superchem.com.au"
        self.api_url = self.pharmacy_locations.SUPERCHEM_API_URL
        self.headers = {
    ...
    async def fetch_locations(self):
        """
        Fetch all SuperChem Pharmacy locations from the API
        
        Returns:
            List of dictionaries containing basic pharmacy information with URLs
        """
        self.logger.info("Fetching SuperChem Pharmacy locations")
                ...
                try:
                    ...
                    if 'items' in data:
                        ...
                        for i, item in enumerate(data['items']):
                            try:
                                store_id = item.get('shopId', f"superchem-{i+1}")
                                ...
                                location = {
                                    ...
                                    'name': store_name,
                                    'url': store_url,
                                    'brand': 'SuperChem Pharmacy',
                                    'raw_data': item  # Store the raw data for later use
                                }
                                
                                all_locations.append(location)
                            except Exception as e:
                                self.logger.warning(f"Error extracting SuperChem Pharmacy location item {i}: {str(e)}")
                        
                        self.logger.info(f"Found {len(all_locations)} SuperChem Pharmacy locations")
                        return all_locations
                    else:
                        self.logger.error("Missing 'items' key in SuperChem Pharmacy API response")
                        return []
                except json.JSONDecodeError:
                    self.logger.error("Failed to decode SuperChem Pharmacy API response as JSON")
                    return []
            else:
                self.logger.error(f"Failed to fetch SuperChem Pharmacy locations: {response.status_code}")
                return []
        except Exception as e:
            self.logger.error(f"Exception when fetching SuperChem Pharmacy locations: {str(e)}")
            return []
    ...
    async def fetch_pharmacy_details(self, location):
        """
        Fetch detailed information for a specific SuperChem Pharmacy
    ...
    async def fetch_all_locations_details(self):
        """
        Fetch details for all SuperChem Pharmacy locations
        
        Returns:
            List of dictionaries containing pharmacy details
        """
        self.logger.info("Fetching details for all SuperChem Pharmacy locations")
        
        # First, get all locations with their URLs
        locations = await self.fetch_locations()
        
        if not locations:
            self.logger.warning("No SuperChem Pharmacy locations found")
            return []
        ...
        self.logger.info(f"Successfully fetched details for {len(all_details)} SuperChem Pharmacy locations")
        return all_details
    ...
    def _parse_detail_page(self, location, html_content):
        ...
        if raw_data:
            ...
            if 'address' in raw_data:
                ...
                # Example format: "Kinross Central Shopping Centre, 3 Selkirk Drive, Kinross 6028, Western Australia"
