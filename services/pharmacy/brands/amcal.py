...
class AmcalHandler(BasePharmacyHandler):
    """Handler for Amcal Pharmacies"""
    
    def __init__(self, pharmacy_locations):
        super().__init__(pharmacy_locations)
        self.brand_name = "amcal"
        self.brand_config = pharmacy_locations.BRAND_CONFIGS["amcal"]
        
    async def fetch_locations(self):
        """Fetch all Amcal locations"""
        ...
        if response.status_code == 200:
            ...
            raise Exception(f"Failed to fetch Amcal locations: {response.status_code}")
    
    async def fetch_pharmacy_details(self, location_id):
        """
        Fetch detailed information for a specific Amcal pharmacy
    ...
    async def fetch_all_locations_details(self):
        """
        Fetch details for all Amcal locations and return as a list
        ...
        print(f"Completed fetching details for {len(all_details)} Amcal locations.")
        return all_details
