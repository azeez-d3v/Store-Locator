...
class NovaHandler(BasePharmacyHandler):
    """Handler for Nova Pharmacies"""
    
    def __init__(self, pharmacy_locations):
        super().__init__(pharmacy_locations)
        self.brand_name = "nova"
    ...
    async def fetch_locations(self):
        """
        Fetch all Nova pharmacy locations.
        
        Returns:
            List of Nova locations
        """
        # Make API call to get location data
        response = await self.session_manager.get(
            url=self.pharmacy_locations.NOVA_URL,
        ...
        if response.status_code == 200:
            ...
            print(f"Found {len(locations)} Nova Pharmacy locations")
            return locations
        else:
            raise Exception(f"Failed to fetch Nova Pharmacy locations: {response.status_code}")
    
    async def fetch_pharmacy_details(self, location_id):
        """
        Fetch details for a specific pharmacy location.
        For Nova Pharmacy, all details are included in the main API call.
        ...
        """
        # All details are included in the locations response for Nova
    ...
    async def fetch_all_locations_details(self):
        ...
        """
        print(f"Fetching all Nova Pharmacy locations...")
        locations = await self.fetch_locations()
        if not locations:
            print(f"No Nova Pharmacy locations found.")
            return []
            
        print(f"Found {len(locations)} Nova Pharmacy locations. Processing details...")
        all_details = []
        ...
        for location in locations:
            try:
                ...
                print(f"Error processing Nova Pharmacy location {location.get('id')}: {e}")
                
        print(f"Completed processing details for {len(all_details)} Nova Pharmacy locations.")
        return all_details
