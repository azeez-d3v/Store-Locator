...
class PharmacistAdviceHandler(BasePharmacyHandler):
    """Handler for Pharmacist Advice stores"""
    
    def __init__(self, pharmacy_locations):
        super().__init__(pharmacy_locations)
        self.brand_name = "pharmacist_advice"
    ...
    async def fetch_locations(self):
        """
        Fetch all Pharmacist Advice pharmacy locations.
        
        Returns:
            List of Pharmacist Advice locations
        """
        # Make API call to get location data
        response = await self.session_manager.get(
            url=self.pharmacy_locations.PHARMACIST_ADVICE_URL,
        ...
        if response.status_code == 200:
            ...
            try:
                ...
                print(f"Found {len(locations)} Pharmacist Advice locations")
                return locations
            except Exception as e:
                print(f"Error parsing Pharmacist Advice XML response: {e}")
                return []
        else:
            raise Exception(f"Failed to fetch Pharmacist Advice locations: {response.status_code}")
    def _parse_xml_response(self, xml_content):
        """
        Parse the XML response from Pharmacist Advice API
    ...
    async def fetch_pharmacy_details(self, location_id):
        ...
        For Pharmacist Advice, all details are included in the main API call.
        ...
        """
        # All details are included in the locations response for Pharmacist Advice
    ...
    async def fetch_all_locations_details(self):
        ...
        """
        print("Fetching all Pharmacist Advice locations...")
        locations = await self.fetch_locations()
        if not locations:
            print("No Pharmacist Advice locations found.")
            return []
            
        print(f"Found {len(locations)} Pharmacist Advice locations. Processing details...")
        all_details = []
        ...
        for location in locations:
            try:
                ...
                print(f"Error processing Pharmacist Advice location {location.get('id')}: {e}")
                
        print(f"Completed processing details for {len(all_details)} Pharmacist Advice locations.")
        return all_details
    def extract_pharmacy_details(self, pharmacy_data):
        ...
        # Parse address - Pharmacist Advice uses a single address field
        ...
        if full_address:
            # Try to parse the address format used by Pharmacist Advice
