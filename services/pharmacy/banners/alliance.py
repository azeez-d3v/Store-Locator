...
class AlliancePharmacyHandler(BasePharmacyHandler):
    """Handler for Alliance Pharmacies"""
    
    def __init__(self, pharmacy_locations):
        super().__init__(pharmacy_locations)
        self.brand_name = "alliance"
        # Define payload for the API request - different from Advantage
        self.payload = {
            "businessid": "17,18,50,68",
            "source": "IPA-Website",
    ...
    async def fetch_locations(self):
        """
        Fetch all Alliance Pharmacy locations.
        
        Returns:
            List of Alliance Pharmacy locations
        """
        ...
        if response.status_code == 200:
            ...
            print(f"Found {len(locations)} Alliance Pharmacy locations")
            return locations
        else:
            raise Exception(f"Failed to fetch Alliance Pharmacy locations: {response.status_code}")
    
    async def fetch_pharmacy_details(self, location_id):
        """
        Fetch details for a specific pharmacy location.
        For Alliance Pharmacy, all details are included in the main API call.
        ...
        """
        # All details are included in the locations response for Alliance Pharmacy
    ...
    async def fetch_all_locations_details(self):
        ...
        """
        print("Fetching all Alliance Pharmacy locations...")
        locations = await self.fetch_locations()
        if not locations:
            print("No Alliance Pharmacy locations found.")
            return []
            
        print(f"Found {len(locations)} Alliance Pharmacy locations. Processing details...")
        all_details = []
        ...
        for location in locations:
            try:
                ...
                print(f"Error processing Alliance Pharmacy location '{location_name}': {e}")
                
        print(f"Completed processing details for {len(all_details)} Alliance Pharmacy locations.")
        return all_details
    ...
    def _parse_trading_hours(self, hours_data):
        """
        Parse trading hours from the Alliance Pharmacy API format to structured format.
        Format examples: 
