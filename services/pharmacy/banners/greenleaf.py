...
class GreenleafHandler(BasePharmacyHandler):
    """Handler for Greenleaf Pharmacies"""
    
    def __init__(self, pharmacy_locations):
        super().__init__(pharmacy_locations)
        self.brand_name = "greenleaf"
        ...
        self.headers = {
            ...
            'pragma': 'no-cache',
            'priority': 'u=1, i',
            'referer': 'https://greenleafpharmacies.com.au/stores/',
    ...
    async def fetch_locations(self):
        """
        Fetch all Greenleaf pharmacy locations.
        
        Returns:
            List of Greenleaf locations
        """
        # Make API call to get location data
        response = await self.session_manager.get(
            url=self.pharmacy_locations.GREENLEAF_URL,
        ...
        if response.status_code == 200:
            ...
            print(f"Found {len(locations)} Greenleaf Pharmacy locations")
            return locations
        else:
            raise Exception(f"Failed to fetch Greenleaf Pharmacy locations: {response.status_code}")
    
    async def fetch_pharmacy_details(self, location_id):
        """
        Fetch details for a specific pharmacy location.
        For Greenleaf Pharmacy, all details are included in the main API call.
        ...
        """
        # All details are included in the locations response for Greenleaf
    ...
    async def fetch_all_locations_details(self):
        ...
        """
        print("Fetching all Greenleaf Pharmacy locations...")
        locations = await self.fetch_locations()
        if not locations:
            print("No Greenleaf Pharmacy locations found.")
            return []
            
        print(f"Found {len(locations)} Greenleaf Pharmacy locations. Processing details...")
        all_details = []
        ...
        for location in locations:
            try:
                ...
                print(f"Error processing Greenleaf Pharmacy location {location.get('id')}: {e}")
                
        print(f"Completed processing details for {len(all_details)} Greenleaf Pharmacy locations.")
        return all_details
