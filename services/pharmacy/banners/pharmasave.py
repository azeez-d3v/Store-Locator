...
class PharmasaveHandler(BasePharmacyHandler):
    ...
    def __init__(self, pharmacy_locations):
        ...
        self.headers = {
            ...
            'pragma': 'no-cache',
            'priority': 'u=1, i',
            'referer': 'https://www.pharmasave.com.au/stores/',
    ...
    async def fetch_locations(self):
        """
        Fetch all Pharmasave pharmacy locations.
        
        Returns:
            List of Pharmasave locations
        """
        # Make API call to get location data
        response = await self.session_manager.get(
            url=self.pharmacy_locations.PHARMASAVE_URL,
        ...
        if response.status_code == 200:
            ...
            print(f"Found {len(locations)} Pharmasave locations")
            return locations
        else:
            raise Exception(f"Failed to fetch Pharmasave locations: {response.status_code}")
    
    async def fetch_pharmacy_details(self, location_id):
        """
        Fetch details for a specific pharmacy location.
        For Pharmasave, all details are included in the main API call.
        ...
        """
        # All details are included in the locations response for Pharmasave
    ...
    async def fetch_all_locations_details(self):
        ...
        for location in locations:
            try:
                ...
                print(f"Error processing Pharmasave location {location.get('id')}: {e}")
                
        print(f"Completed processing details for {len(all_details)} Pharmasave locations.")
        return all_details
