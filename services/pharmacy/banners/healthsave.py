...
class HealthsaveHandler(BasePharmacyHandler):
    """Handler for Healthsave Pharmacies"""
    
    def __init__(self, pharmacy_locations):
        super().__init__(pharmacy_locations)
        self.brand_name = "healthsave"
        ...
        self.headers = {
            ...
            'pragma': 'no-cache',
            'priority': 'u=1, i',
            'referer': 'https://www.healthsave.com.au/stores/',
    ...
    async def fetch_locations(self):
        """
        Fetch all Healthsave pharmacy locations.
        
        Returns:
            List of Healthsave locations
        """
        # Make API call to get location data
        response = await self.session_manager.get(
            url=self.pharmacy_locations.HEALTHSAVE_URL,
        ...
        if response.status_code == 200:
            ...
            print(f"Found {len(locations)} Healthsave locations")
            return locations
        else:
            raise Exception(f"Failed to fetch Healthsave locations: {response.status_code}")
    
    async def fetch_pharmacy_details(self, location_id):
        """
        Fetch details for a specific pharmacy location.
        For Healthsave, all details are included in the main API call.
        ...
        """
        # All details are included in the locations response for Healthsave
    ...
    async def fetch_all_locations_details(self):
        ...
        """
        print("Fetching all Healthsave locations...")
        locations = await self.fetch_locations()
        if not locations:
            print("No Healthsave locations found.")
            return []
            
        print(f"Found {len(locations)} Healthsave locations. Processing details...")
        all_details = []
        ...
        for location in locations:
            try:
                ...
                print(f"Error processing Healthsave location {location.get('id')}: {e}")
                
        print(f"Completed processing details for {len(all_details)} Healthsave locations.")
        return all_details
