...
class JadinHandler(BasePharmacyHandler):
    """Handler for Jadin Chemist Group"""
    
    def __init__(self, pharmacy_locations):
        super().__init__(pharmacy_locations)
        self.brand_name = "jadin"
        ...
        self.headers = {
            ...
            'pragma': 'no-cache',
            'priority': 'u=1, i',
            'referer': 'https://jadin.com.au/stores/',
    ...
    async def fetch_locations(self):
        """
        Fetch all Jadin Chemist Group pharmacy locations.
        
        Returns:
            List of Jadin locations
        """
        # Make API call to get location data
        response = await self.session_manager.get(
            url=self.pharmacy_locations.JADIN_URL,
        ...
        if response.status_code == 200:
            ...
            print(f"Found {len(locations)} Jadin Chemist Group locations")
            return locations
        else:
            raise Exception(f"Failed to fetch Jadin Chemist Group locations: {response.status_code}")
    
    async def fetch_pharmacy_details(self, location_id):
        """
        Fetch detailed information for a specific Jadin pharmacy.
        ...
        """
        # For Jadin, the main location data usually contains all necessary details
    ...
    def transform_location_data(self, location):
        """
        Transform Jadin location data to standardized format.
        
        Args:
            location: Raw location data from Jadin API
        ...
        try:
            ...
            return {
                ...
                'hours': hours,
                'url': url,
                'brand': 'Jadin Chemist Group'
            }
            
        except Exception as e:
            print(f"Error transforming Jadin location data: {e}")
            return None
    
    def format_hours(self, hours_data):
        """
        Format Jadin hours data into a readable string.
        ...
        try:
            ...
            print(f"Error formatting Jadin hours: {e}")
            return "Hours formatting error"
    
    def clean_phone_number(self, phone):
        """
        Clean and format Jadin phone number.
    ...
    def extract_store_id(self, location):
        """
        Extract store ID from Jadin location data.
    ...
    async def get_all_locations(self):
        """
        Get all Jadin Chemist Group pharmacy locations in standardized format.
        ...
        try:
            ...
            if not raw_locations:
                print("No Jadin Chemist Group locations found")
                return []
            ...
            print(f"Successfully processed {len(standardized_locations)} Jadin Chemist Group locations")
            return standardized_locations
            
        except Exception as e:
            print(f"Error getting Jadin Chemist Group locations: {e}")
            return []
    ...
    async def fetch_all_locations_details(self):
        ...
        """
        print("Fetching all Jadin Chemist Group locations...")
        locations = await self.fetch_locations()
        if not locations:
            print("No Jadin Chemist Group locations found.")
            return []
            
        print(f"Found {len(locations)} Jadin Chemist Group locations. Processing details...")
        all_details = []
        ...
        for location in locations:
            try:
                ...
                print(f"Error processing Jadin location {location.get('id')}: {e}")
                
        print(f"Completed processing details for {len(all_details)} Jadin Chemist Group locations.")
        return all_details
    ...
    def _parse_trading_hours(self, hours_html):
        ...
        try:
            ...
            print(f"Error parsing Jadin trading hours: {e}")
            return "Hours not available"
